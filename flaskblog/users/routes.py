    ...
    if not(current_user.is_authenticated):
        ...
        if form.validate_on_submit():
            ...
            if user and bcrypt.check_password_hash(user.password, form.password.data):
                ...
                return redirect(url_for(next.strip("/"))) if next else redirect(url_for("main.home"))
            else:
...
def account():
    ...
    if form.validate_on_submit():
        if form.picture.data:
            current_user.user_image = save_picture(form.picture.data)
        ...
        form.about.data = current_user.email
    user_image = url_for("static", filename="dp/" + current_user.user_image)
...
def user_posts(username):
    ...
    posts = Post.query.filter_by(author=user).order_by(
        Post.date_posted.desc()).paginate(per_page=5, page=1)
